...
import { acquireLock } from "../socket/socketoutgoing"
    ...
    useEffect(() => {
        ...
        if (getSession().editorRef != null) {
            ...
            editor.onDidChangeCursorPosition(e => {
                //if the current pos of the cursor is not locked then allow the user to edit the line
                if (!getSession().lockedlines.has(editor.getPosition().lineNumber)) {
                    console.log('curlock')
                    return
                }
                ...
                if (lockedlines.has(e.position.lineNumber)) {
                    ...
                    if (e.position.lineNumber > 1) {
                        ...
                        acquireLock(getSession().socket, getSession().sessionID, e.position.lineNumber - 1)
                    }
